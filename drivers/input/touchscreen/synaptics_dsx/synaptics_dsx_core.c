...
static struct proc_dir_entry *syna_info_proc_entry;
extern unsigned char *hq_fwu_config_id;

static ssize_t syna_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	char buf[150] = {0};
	int retval = 0;
	if(!hq_fwu_config_id){
		printk("syna hq_fwu_config_id is NULL\n");
		return  -1;
	}else{
		snprintf(buf, 150, "IC=S3706 module=Samsung fw_ver= %02x%02x%02x%02x \n",hq_fwu_config_id[0],hq_fwu_config_id[1],hq_fwu_config_id[2],hq_fwu_config_id[3]);
		retval = simple_read_from_buffer(buff, size, pPos, buf, strlen(buf));
	}
	return retval;
}

static const struct file_operations syna_info_proc_fops = {
	.owner = THIS_MODULE,
	.read = syna_proc_getinfo_read,
};

int32_t syna_extra_proc_init(void)
{
	syna_info_proc_entry = proc_create(SYNA_INFO_PROC_FILE, 0777, NULL, &syna_info_proc_fops);
	if (NULL == syna_info_proc_entry)
	{
		printk( "[synaptics] %s() Couldn't create proc entry!",__func__);
		return -ENOMEM;
	}else	{
		printk( "[synaptics] %s() Create proc entry success!",__func__);
	}
	return 0;
}
#endif
...
#if 0
void synaptics_rmi4_report_rate240_switch(struct synaptics_rmi4_data *rmi4_data,
                bool enable)
{
        int retval;
        unsigned short f54_analog_ctrl95 = 0x0127;
	unsigned char f54_analog_ctrl95_data240[88] = {0x83, 0x2e, 0x00, 0x0e, 0x00, 0x00, 0x00, 0x04, 0x00, 0x01,0x24, 0x83, 0x2e, 0x00, 0x0e, 0x00, 0x06, 0x00, 0x09, 0x00, 0x02, 0x24, 0x83, 0x2e, 0x00, 0x0d, 0x00, 0x0c, 0x00, 0x05,0x00, 0x01, 0x23, 0x83, 0x2e, 0x00, 0x0d, 0x00, 0x27, 0x00, 0x0d, 0x00, 0x02, 0x23, 0x01, 0x1c, 0x00, 0x0f, 0x00, 0x00, 0x00, 0x04, 0x00, 0x01, 0x25, 0x01, 0x1c, 0x00, 0x0e, 0x00, 0x06, 0x00, 0x09, 0x00, 0x02, 0x24, 0x01, 0x1c, 0x00, 0x0c, 0x00, 0x0c, 0x00, 0x05, 0x00, 0x01, 0x23, 0x01, 0x1c, 0x00, 0x0c, 0x00, 0x23, 0x00, 0x06, 0x00, 0x01, 0x25};
/*
	unsigned char f54_analog_ctrl95_data120[88] ={0x3, 0x2e, 0x0, 0xe, 0x0, 0x0, 0x0, 0x4, 0x0, 0x1,0x24, 0x3, 0x2e, 0x0, 0xe, 0x0, 0x6, 0x0, 0x9, 0x0, 0x2, 0x24, 0x3, 0x2e, 0x0, 0xd, 0x0, 0xc, 0x0, 0x5,0x0, 0x1, 0x22, 0x3, 0x2e, 0x0, 0xd, 0x0, 0x27, 0x0, 0xd, 0x0, 0x2, 0x23, 0x83, 0x19, 0x0, 0xe, 0x0, 0x0,0x0, 0x4, 0x0, 0x1, 0x24, 0x83, 0x19, 0x0, 0xe, 0x0, 0x6, 0x0, 0x9, 0x0, 0x2, 0x24, 0x83, 0x19, 0x0, 0xd,0x0, 0xc, 0x0, 0x5, 0x0, 0x1, 0x22, 0x83, 0x19, 0x0, 0xd, 0x0, 0x1e, 0x0, 0x6, 0x0, 0x1, 0x23};
*/
	unsigned short f01_rmi_cmd00 = 0x0026;
	unsigned char f01_rmi_cmd00_data[1] = {0x01};

	unsigned short f54_analog_cmd00 = 0x0141;
	unsigned char f54_analog_cmd00_data[1] = {0x04};
/*
        unsigned short f12_2d_ctrl15 = 0x0019;
        unsigned char f12_2d_ctrl15_data[1] = {0x3b};

        unsigned short f54_analog_ctrl199 = 0x013F;
        unsigned char f54_analog_ctrl199_data[1] = {0x01};
*/

	unsigned short f51_custom_ctrl0404 = 0x0416;
        unsigned char f51_custom_ctrl0404_data[1] = {0xbe};

	unsigned short f54_analog_ctrl146 = 0x0136;
	unsigned char f54_analog_ctrl146_data[7] = {0x50, 0x00, 0x07, 0x01, 0x00, 0x19, 0x12};
	...
	if (enable) {
		// soft reset
                retval = synaptics_rmi4_reg_write(rmi4_data,
                        f01_rmi_cmd00,
                        f01_rmi_cmd00_data,
                        sizeof(f01_rmi_cmd00_data));

                if (retval < 0) {
                        printk("syna %s: Failed to change reporting rate, write 0x0026 reg error\n",
                                __func__);
                        return;
                }
		...
		retval = synaptics_rmi4_reg_write(rmi4_data,
                        f54_analog_ctrl95,
                        f54_analog_ctrl95_data240,
                        sizeof(f54_analog_ctrl95_data240));

		if (retval < 0) {
			printk("syna %s: Failed to change reporting rate, write 0x0127 reg error\n",
				__func__);
			return;
		}
                ...
                retval = synaptics_rmi4_reg_write(rmi4_data,
                        f54_analog_ctrl146,
                        f54_analog_ctrl146_data,
                        sizeof(f54_analog_ctrl146_data));

                if (retval < 0) {
                        printk("syna %s: Failed to change reporting rate, write 0x0136 reg error\n",
                                __func__);
                        return;
                }
/*
                retval = synaptics_rmi4_reg_write(rmi4_data,
                        f12_2d_ctrl15,
                        f12_2d_ctrl15_data,
                        sizeof(f12_2d_ctrl15_data));

                retval = synaptics_rmi4_reg_write(rmi4_data,
                        f54_analog_ctrl199,
                        f54_analog_ctrl199_data,
                        sizeof(f54_analog_ctrl199_data));
*/

		retval = synaptics_rmi4_reg_write(rmi4_data,
                        f51_custom_ctrl0404,
                        f51_custom_ctrl0404_data,
                        sizeof(f51_custom_ctrl0404_data));

		//force update
		retval = synaptics_rmi4_reg_write(rmi4_data,
                        f54_analog_cmd00,
                        f54_analog_cmd00_data,
                        sizeof(f54_analog_cmd00_data));

                if (retval < 0) {
                        printk("syna %s: Failed to change reporting rate, write 0x0141 reg error\n",
                                __func__);
                        return;
                }

		printk("syna synaptics_rmi4_report_rate240_switch set report rate 240hz\n");
	}
	else {
                // soft reset
                retval = synaptics_rmi4_reg_write(rmi4_data,
		...
		printk("syna synaptics_rmi4_report_rate240_switch set report rate 120hz\n");
	}
                ...
                bool enable)
{
	...
	unsigned short f51_custom_ctrl49 = 0x043A;
	unsigned char f51_custom_ctrl49_data[1] = {0x01};
	...
	if (enable) {
                ...
                retval = synaptics_rmi4_reg_write(rmi4_data,
                        f51_custom_ctrl49,
                        f51_custom_ctrl49_data,
                        sizeof(f51_custom_ctrl49_data));

                if (retval < 0) {
                        printk("syna %s: Failed to change reporting rate, write 0x043A reg error\n",
                                __func__);
                        return;
                }
...
#define ZUI_NUM_OF_SLOT 3
#define SYNAPTICS_FP_MATCH 10
#define SYNAPTICS_FP_NOT_MATCH 2
static int synaptics_fp_match = 0;
...
/*HQ modifiy for JD2020-428 finger down by limengxia at 2019/03/07 strat*/
	if (1) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				finger_reg,1);
		if (retval < 0){
			printk("synaptics TP read finger_reg fail\n");
		}else{
			if((int)(finger_reg[0])==0x0f){
				hq_finger_down=1;
				synaptics_fp_match = 1;
				printk("synaptics TP finger down:%d\n",hq_finger_down);
			}else if((int)(finger_reg[0])==0x1f){
				hq_finger_down=0;
				synaptics_fp_match = 0;
				printk("synaptics TP finger up:%d\n",hq_finger_down);
			}
		}
	}
/*HQ modifiy for JD2020-428 finger down by limengxia at 2019/03/07 end*/
...
<<<<<<< HEAD
				...
				#else
				if (synaptics_fp_match)
					input_report_abs(rmi4_data->input_dev,
							ABS_MT_TOUCH_MAJOR, SYNAPTICS_FP_MATCH);
				else
					input_report_abs(rmi4_data->input_dev,
...
/* Haptic TP event notify start */
			...
			rmi4_data->tp_event.record_point[finger].status = 1;
			rmi4_data->tp_event.record_point[finger].x = x;
			rmi4_data->tp_event.record_point[finger].y = y;
...
/* Haptic TP event notify start */
			dev_dbg(rmi4_data->pdev->dev.parent,
					"finger =%d up\n", finger);
			rmi4_data->tp_event.i = finger;
			rmi4_data->tp_event.record_point[finger].status = 0;
...
/* Haptic TP event notify start */
		...
		rmi4_data->tp_event.record_point[0].status = 0;
...
<<<<<<< HEAD
	...
	printk("syna zui_num_of_fingers %d\n", zui_num_of_fingers);

#ifdef KERNEL_ABOVE_3_6
	input_mt_init_slots(rmi4_data->input_dev,
			zui_num_of_fingers, INPUT_MT_DIRECT);
#else
	input_mt_init_slots(rmi4_data->input_dev,
			zui_num_of_fingers);
=======
...
#endif
#ifdef ESD_WATCHDOG
	bool esd_watchdog = false;
	char data_value = 0;
	char predata_value = 0;
static void synaptics_rmi4_esd_watchdog_work(struct work_struct *work)
{
	struct delayed_work *delayed_work =
			container_of(work, struct delayed_work, work);
	struct synaptics_rmi4_data *rmi4_data =
	...
	if(!hq_fwu_config_id){
		printk("syna hq_fwu_config_id is NULL\n");
		return;
	}else if (hq_fwu_config_id[3] < 0x14){
		printk("[syna]hq add the fw version is %x ,is old fw dont run esd\n",hq_fwu_config_id[3]);
		return;
	}

	synaptics_rmi4_reg_read(rmi4_data, 0x0400, &data_value, sizeof(data_value));
	printk("[syna]hq add for predata_value = %d,data_value = %d \n",predata_value,data_value);
	if ((predata_value != data_value) || (data_value == 0 && esd_watchdog == false)) {
		predata_value = data_value;
		esd_watchdog = true;
	} else {
		printk("hq add for esd tp reset start \n");
		if (bdata->reset_gpio >= 0) {
			gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
			msleep(bdata->reset_active_ms);
			gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
		...
		printk("hq add for esd tp reset end \n");
	}
	queue_delayed_work(rmi4_data->esd_watchdog_workqueue, &rmi4_data->esd_watchdog_work,msecs_to_jiffies(ESD_TIMER));
	return;
}
#endif
...
extern   ssize_t ito_test(struct device *dev,struct device_attribute *attr, char *buf);
static ssize_t ito_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	int count;
	if(ito_test(dev,attr,buf)==0){
...
int syna_test_node_init(struct platform_device *tpinfo_device)
{
    ...
    if (0 != err)
    {
        printk( "[syna-ito] %s() - ERROR: sysfs_create_group() failed.",  __func__);
        sysfs_remove_group(&tpinfo_device->dev.kobj, &ito_test_attribute_group);
        return -EIO;
    }
    else
    {
        printk("[syna-ito] %s() - sysfs_create_group() succeeded.", __func__);
    }
    return err;
}

void synaptics_gesture_en(int enable)
{
        struct synaptics_rmi4_data *rmi4_data;
...
static int synaptics_report_rate240 = 0;
void synaptics_report_rate_switch(int enable)
{
        struct synaptics_rmi4_data *rmi4_data;
	...
	if(enable)
		synaptics_report_rate240 = 1;
	else
		synaptics_report_rate240 = 0;

        synaptics_rmi4_report_rate240_switch(rmi4_data, synaptics_report_rate240);
}

static struct synaptics_rmi4_data *zui_rmi4_data;
...
*/
	rmi4_data->drm_notifier.notifier_call =
					synaptics_rmi4_drm_notifier_cb;
	if (active_panel) {
...
<<<<<<< HEAD
	...
	INIT_DELAYED_WORK(&rmi4_data->esd_watchdog_work, synaptics_rmi4_esd_watchdog_work);
	queue_delayed_work(rmi4_data->esd_watchdog_workqueue, &rmi4_data->esd_watchdog_work,msecs_to_jiffies(ESD_TIMER));
	//printk("[syna]hq add for esd work start\n");
#endif
#if SYNA_TOUCH_EXT_PROC
	syna_extra_proc_init();
#endif
...
void zui_input_report_atr_key_base(int x, int y, int press, int slot_flag)
{
    ...
    struct input_dev *input_dev = zui_rmi4_data->input_dev;
	int num_of_slot = 0;

        printk("zui_input_report_atr_key press %d, x %d, y %d, slot_flag %d\n", press, x, y, slot_flag);

        mutex_lock(&zui_rmi4_data->rmi4_report_mutex);
	switch (slot_flag) {
		case 0 :
	...
	if ((zui_atr_key_touch_0 == 0) && (zui_atr_key_touch_1 == 0) && (zui_atr_key_touch_2 == 0))
     ...
     mutex_unlock(&zui_rmi4_data->rmi4_report_mutex);
}
